...
print(f"env key: {os.environ.get('APP_SECRET_KEY')}")

app = Flask(__name__)
app.app_context().push()
app.config['SECRET_KEY'] = os.environ.get('APP_SECRET_KEY')  # '8BYkEfBA6O6donzWlSihBXox7C0sKR6b'
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("Email adress allready registered. Please log in instead.")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        if not current_user.is_authenticated:
            flash("You need to be signed in to make comments!")
            return redirect(url_for("login"))
        user = current_user
